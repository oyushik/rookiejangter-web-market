...
public class DibsDTO {
    ...
    public static class Response {
        private Long productId;
        private boolean isDibbed;  // 요청하는 사용자 ID와 상품 ID를 기반으로 Dibs 테이블에 해당 레코드가 존재하는지 확인합니다.
        private LocalDateTime dibbedAt;

        public static Response fromEntity(Dibs dibs, boolean isDibbed) {
            return Response.builder()
                    .productId(dibs.getProduct().getProductId())
                    .isDibbed(isDibbed)
                    .dibbedAt(dibs.getCreatedAt())
                    .build();
            }
        }
        // 찜한 상품 목록 조회를 위한 응답
